...
local function Trim( sInput )
	if type( sInput ) == "number" then return tostring(sInput) end
...
local function ParseWhere( sQuery, tWhere )
	...
	if #tInput == 0 then return sQuery end
	for tKey, tValue in ipairs( tInput ) do
...
function Compile.UPDATE( Object )
	...
	local sReturn, tSet = "UPDATE " .. Object:escapeName(Object._from) .. "\nSET ", {}
	for Key, Value in pairs( Object._update ) do
		tSet[#tSet + 1] = Object:escapeName(Key) .. " = " .. Object:escapeValue(Value)
	end
...
function Compile.INSERT( Object )
	...
	return ("INSERT INTO %s (%s)\nVALUES\n(\n\t%s\n)"):format(
		Object:escapeName(tInsert.tbl),
		table.concat(tInsert.cols, ", "),
		table.concat(tInsert.val, ",\n\t")
	)
end
...
function luaqub:wherePlain( clauses, value, joiner )
	self:where(clauses, value, joiner, 1)
end

function luaqub:where( clauses, value, joiner, isPlain )
	if not clauses then
	...
	if type(clauses) == "string" then
		if type(value) ~= "number" then
			if not value then
				value = "IS NULL"
				clauses = clauses:gsub("[=><]+", "")
			elseif value == true then
				value = "IS NOT NULL"
				clauses = clauses:gsub("[=><]+", "")
			--elseif type(value) == "table" then
			--	if #value < 1 then throw(clauses .. " value is empty-table.") end
			--	value = table.concat(",", self:escapeValue(value))
			elseif not isPlain then
				if type(value) == "table" then
					if #value < 1 then throw(clauses .. " value is empty-table.") end
					for i=1, #value do
						value[i] = self:escapeValue(value[i])
					end
				else
		...
		local statement = Trim(clauses)
		if type(value) == "table" then
			statement = statement .. "(" .. table.concat(value, ",") .. ")"
		else
		...
		for Key, Value in pairs( clauses ) do
			if tonumber( Key ) then
				...
				table.insert(
					...
					{
						join = joiner,
						statement = ("%s = %s"):format(Trim(Key), Trim(Value))
					}
				)
			end
		end
...
function luaqub:escapeValue(value)
	if type(value) == "number" then
...
function luaqub:insert( tbl, datas )
	...
	for Key, Value in pairs( datas ) do
		table.insert(tCols,  self:escapeName(Trim(Key)))
		table.insert(tValue, self:escapeValue(Value))
	end
