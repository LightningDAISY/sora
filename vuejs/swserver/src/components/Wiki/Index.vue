  ...
  <section class="margin-top-large">
    ...
    <div class="columns">
      ...
      <div class="column is-two-quater">
        <div class="card" v-for="entry in frontEntries">
          <header class="card-header">
            <p class="card-header-title">{{ entry.subject }}</p>
          </header>
          <div class="card-content">
            <div class="content wiki-format" v-html="generateHtml(entryDigest(entry.body))"></div>
          </div>
          <footer class="card-footer">
            <a :href="'#/wiki/entry/' + entry.id" class="card-footer-item">read more</a>
            <a :href="'#/wiki/edit/' + entry.id" class="card-footer-item">Edit</a>
            <a class="card-footer-item" @click="deleteEntry(entry)">Delete</a>
          </footer>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    generateHtml: function (struct) {
      return WikiGenerator.generate(struct)
    },
    entryDigest: function (json) {
      let struct = JSON.parse(json)
      struct = Array.prototype.slice.call(struct,0,2)
      return struct
    },
	...
	deleteEntry: function(entry) {
      ...
      this.basicRequest(
        ...
        function(json) {
          if(json.result == 'OK') {
    		that.setAllEntry()
            that.modalMessage = 'removed the entry.'
